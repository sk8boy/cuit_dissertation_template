...
Const TEXT_AppName = "³É¶¼ÐÅÏ¢¹¤³Ì´óÑ§Ë¶Ê¿Ñ§Î»ÂÛÎÄÄ£°å"
Const TEXT_Author = "ÍõÌú¾ü @ ³É¶¼ÐÅÏ¢¹¤³Ì´óÑ§ ¼ÆËã»úÑ§Ôº"
Const TEXT_Description = "ÎªÊ¹ÓÃ Word »ò WPS ×«Ð´Ë¶Ê¿Ñ§Î»ÂÛÎÄµÄÍ¬Ñ§Ìá¹©Ò»¸ö¿ìËÙÉÏÊÖµÄÄ£°å¡£"
Const TEXT_VersionPrompt = "°æ±¾£º"
Const TEXT_NonCommecialPrompt = "½öÏÞ·ÇÉÌÒµÓÃÍ¾"
    ...
    For Each fld In ActiveDocument.Fields
        If fld.Type = wdFieldDocVariable Then
            ...
            If InStr(1, fld.Code.text, "BodyPageCount", vbTextCompare) > 0 Then
                fld.Update
                ' ¿ÉÒÔÔÚ´Ë´¦¸üÐÂ±äÁ¿µÄÖµ
                ' ActiveDocument.Variables("MyVariable").Value = "ÐÂÖµ"
            End If
        End If
    Next fld
...
Public Sub InsertChapterSep(ByVal control As IRibbonControl)
    ...
    ElseIf mathTypeFound Then
        MsgBox "ÇëÊ¹ÓÃAxMath²Ëµ¥ÏÂµÄÕÂ&½Ú¹¦ÄÜ£¬ÔÚÃ¿ÕÂ¿ªÊ¼´¦²åÈëÕÂ·Ö¸ô·û£¡", vbExclamation, C_TITLE
    Else
...
'    axMathFound = True
    ...
    With ActiveDocument
        ...
        Set aField = currentRange.Fields.Add(currentRange, wdFieldEmpty, "SEQ Í¼ \* ARABIC \s 1", False)
        Set aRange = .Range(currentRange.End, currentRange.End)
        aRange.text = "-"
        Set bField = aRange.Fields.Add(currentRange, wdFieldEmpty, "QUOTE ""Ò»¾ÅÒ»Ò»ÄêÒ»ÔÂÈÕ"" \@""D""", False)
        Set aRange = .Range(bField.Code.End - 9, bField.Code.End - 9)
        Set bField = aRange.Fields.Add(aRange, wdFieldEmpty, "STYLEREF ""ÂÛÎÄ±êÌâ1"" \s", False)
    End With
    Selection.TypeText " "
...
ERROR_HANDLER:
    MsgBox "¼ì²éÒ³ÃæÉèÖÃÊ±·¢Éú´íÎó: " & vbCrLf & vbCrLf & Err.Description, vbCritical, C_TITLE
    If Not (ur Is Nothing) Then ur.EndCustomRecord
End Sub
...
Sub OpenCrossReferenceDialog()
    ...
    With Application.Dialogs(wdDialogInsertCrossReference)
        ' ÉèÖÃÄ¬ÈÏ²ÎÊý£¨¿ÉÑ¡£©
        .ReferenceType = "Í¼"  ' ÒýÓÃÀàÐÍ£º±êÌâ¡¢ÊéÇ©¡¢½Å×¢µÈ
'        SendKeys "%r{down 1}", True
'        .ReferenceKind = wdNumberRelativeContext
'        SendKeys "%h", True
        .ReferenceItem = "Í¼1-2 ÂÛÎÄ½á¹¹Í¼"
        .Show
    End With
End Sub
...
Sub NestedField()
    ...
    Set currentRange = Selection.Range
    With ActiveDocument
        Set aField = currentRange.Fields.Add(currentRange, wdFieldEmpty, "MACROBUTTON AMMPlaceRM \* MERGEFORMAT -", False)  '´´½¨¿Õ°×Óò£¬²¢Ð´ÈëÓò´úÂë
        Set aRange = .Range(aField.Code.End - 2, aField.Code.End - 2)
        Set bField = aRange.Fields.Add(aRange, wdFieldEmpty, "SEQ AMEqn \h \* MERGEFORMAT", False)  'ÔÚaRange¶ÔÏóÎ»ÖÃ²åÈëÓò
        Set aRange = .Range(aField.Code.End - 2, aField.Code.End - 2)
        Set bField = aRange.Fields.Add(aRange, wdFieldEmpty, "SEQ AMChap \c \* Arabic \* MERGEFORMAT", False)  'ÔÚaRange¶ÔÏóÎ»ÖÃ²åÈëÓò
        Set aRange = .Range(aField.Code.End - 1, aField.Code.End)
        Set bField = aRange.Fields.Add(aRange, wdFieldEmpty, "SEQ ±í \* ARABIC \s 1", False)  'ÔÚaRange¶ÔÏóÎ»ÖÃ²åÈëÓò
        aField.Update  '¸üÐÂÓò£¬Ð§¹ûµÈÍ¬ÓÚÏÔÊ¾Óò½á¹û
    End With
    Selection.TypeText " "
End Sub
...
Sub FindListTemplateLinkedToHeading()
    ...
    Else
        MsgBox "Î´ÕÒµ½Á´½Óµ½ÑùÊ½ """ & targetStyleName & """ µÄÁÐ±íÄ£°å¡£", vbExclamation, C_TITLE
    End If
End Sub
...
Sub TestStyle()
    ...
    'StyleName = "ÂÛÎÄÌâÄ¿"
    'MsgBox "ÑùÊ½'" & StyleName & "'" & StyleExists(StyleName), vbOKOnly, C_TITLE
    'MsgBox "ÁÐ±íÃû³Æ£º" & ActiveDocument.Styles("±êÌâ 1").ListTemplate.name, vbOKOnly, C_TITLE
